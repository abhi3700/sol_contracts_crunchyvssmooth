...
/// This can be updated via running this `$ solana-keygen new -o target/deploy/crunchy_vs_smooth-keypair.json` in terminal opened in project root directory.

/// Get the program ID (public key) via 
...
/// We'll update our declare_id! later. Once you run `anchor build`, your Program ID (contract account) can be found in /target/idl/[your_project_name].json
